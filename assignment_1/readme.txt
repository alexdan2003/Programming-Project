...
To run pgmTile you require four args the ./pgmTile arg to actually call the program, the input file, the user inputed tile number and the output file. To see this properly simply type  ./pgmTile into the command line and it will show you the correct way to run it.

How pgmTile works

pgmTile works by taking an input file and reading all the valid information into the pgm structure. Follwing this it will split the original pgm up into several smaller images that 
together make up the original. These will all share a naming scheme and can be recombined.
