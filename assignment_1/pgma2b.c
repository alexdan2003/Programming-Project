...
int main(int argc, char **argv){
	...
	//If everything is successful prints CONVERTED and frees the pgm.
	free(pgmAscii->commentLine);
	int height = 0;
	for (height=0; height < pgmAscii->height; height++){
		free(pgmAscii->imageData[height]);
		}
	free(pgmAscii->imageData);
	free(pgmAscii);
	printf("CONVERTED\n");
	return(EXIT_NO_ERRORS);
	}
