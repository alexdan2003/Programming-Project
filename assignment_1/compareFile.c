...
int compareImageData(pgmFile *pgmOne, pgmFile *pgmTwo){
	...
	//Compares the two images image data in ASCII.
	int i = 0;
	for (i=0; i < pgmOne->height; i++){
		int j = 0;
		for (j=0; j < pgmOne->height; j++){
			if (pgmOne->imageData[i][j] != pgmTwo->imageData[i][j]){
				return(1);
				}
			}
		} 
...
int compareFile(pgmFile *pgmOne, pgmFile *pgmTwo){
	...
	int errorCode = compareImageData(pgmOne, pgmTwo);
